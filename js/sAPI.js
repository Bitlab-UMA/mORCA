    ...
    for(var x = 0; x < parameters.length; x++){
            ...
            switch (parameters[x].dataTypeName) {
                ...
                default:
                    ...
                    if(parameters[x].input == "false") { break; }
...
function mainLogin(user, pass){
    if(!logged()){
        ...
        $("#usernamediv").html("");
        $("#usernamediv").append("</br>"+"Welcome, logged is as: "+user+"</br>")
...
$(document).ready(function (){
    if(logged()) {
        var user = getCookie('username');
        $("#usernamediv").html("");
        $("#usernamediv").append("</br>"+"Welcome "+user+"!"+"</br>")
        $('#loginButton').html('Logout');
        $('#loginButton').removeAttr('href');
        $('#loginButton').attr('onclick','mainLogout()');
        loadFileBrowser();
...
function importFile(fileName, type){
    ...
    s3.getObject(
        {Bucket: "morcabucket", Key: fileName },
        function (error, data) {
            if (error != null) {
